...
impl NefFile {
    pub fn open(file_path: &Path) -> Result<NefFile, Error> {
        ...
        let mut meta_data = ImageMetadata { image_size: 0 };

        let mut image_data = ImageData { height: 0 };
    ...
    fn parse_ifds(&self) -> Result<Vec<Ifd>, Error> {
        let mut parsed_ifds = Ifd::parse_ifd(self.buffer.as_slice());
        Ok(parsed_ifds)
    }
...
impl Ifd {
    pub fn parse_ifd(buffer: &[u8]) -> Vec<Self> {
        ...
        if let Some(offset_to_childs) = ifd.get_entry(IfdTag::SubIFDS) {
            ...
            // println!("Could not fetch offset to child IFDs");
        }
